...
(function () {
  ...
  function getBorderWidth(element, side) {
    ...
    if (computed.indexOf('px') < 0) {
      // look up keywords
      if (computed in borderMap && element.getComputedStyle('border-style') != 'none') computed = borderMap[computed];
      else computed = 0;
    }
  ...
  function buildTableColumnPillars(table) {
    var pillars = [],
      ...
      rows = CKEDITOR.tools.array.zip(new CKEDITOR.dom.nodeList(table.$.rows).toArray(), CKEDITOR.tools.buildTableMap(table));
  ...
  function checkWithinDimensions(posX, posY, element) {
    return posX >= element.x && posX <= element.x + element.width && posY >= element.y && posY <= element.y + element.height;
  }
  ...
  function columnResizer(editor) {
      ...
      for (var i = 0; i < cellsCount; i++) {
        ...
        CKEDITOR.tools.setTimeout(
          function (leftCell, leftOldWidth, rightCell, rightOldWidth, tableWidth, sizeShift) {
            ...
            if (tableWidth) table.setStyle('width', pxUnit(tableWidth + sizeShift * (rtl ? -1 : 1)));
    ...
    resizer = CKEDITOR.dom.element.createFromHtml(
      '<div data-cke-temp=1 contenteditable=false unselectable=on ' +
        ...
        'padding:0;background-color:#004;background-image:none;border:0px none;z-index:10000"></div>',
      document
    );
    ...
    move = this.move = function (posX, posY) {
      ...
      if (isResizing) {
        if (resizerNewPosition == leftShiftBoundary || resizerNewPosition == rightShiftBoundary) return 1;
  ...
  function clearPillarsCache(evt) {
    ...
    if (evt.name == 'mouseout') {
      ...
      while (dest && dest.$ && !dest.equals(target) && !dest.is('body')) dest = dest.getParent();
      ...
      editor.on('contentDom', function () {
        ...
        editable.attachListener(listenerTarget, 'scroll', function () {
          ...
          CKEDITOR.tools.array.forEach(
            tables,
            CKEDITOR.tools.debounce(function (table) {
              table.removeCustomData('_cke_table_pillars');
            }, 200)
          );
        });
