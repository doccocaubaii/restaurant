...
var hwcrypto = (function hwcrypto() {
  ...
  function _hex2array(str) {
    if (typeof str == 'string') {
      ...
      for (var i = 0; i < len; i++) {
        ret[i] = parseInt(str.substr(i * 2), 16);
      }
      return ret;
    }
  }
  ...
  const digidoc_ws = 'TokenSigningWS';
  ...
  fields.selectCertSerial = function (options) {
    ...
    if (isChrome) {
      ...
      Promise.reject(new Error('unsupported_browser'));
    }
  };
...
*/
  ...
  fields.hasSigningTool = function () {
    return hasWebsocket();
  };
