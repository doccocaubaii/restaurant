...
import { ImportProductGroupResponse } from './import-product-group-response';
import {
  EXPORT_EXCEL_PRODUCT_GROUP,
  IMPORT_EXCEL_SAVE_PRODUCT_GROUP,
  IMPORT_EXCEL_VALIDATE_PRODUCT_GROUP,
...
@Component({
  ...
  templateUrl: './import-product-group.component.html',
  styleUrls: ['../common/import.component.scss'],
})
export class ImportProductGroupComponent extends BaseComponent implements OnInit {
  private modalRef: NgbModalRef | undefined;
  isLoading = false;
  importStatus: any = [];
  importStatusValue = 0;
  importTypeValue: null;
  urlFileTemplate = 'https://app.easyposs.vn/client/file/excel/product/TaoDanhMucNhomSanPham.xls';
  ...
  dataValidateResponse?: ImportProductGroupResponseData[];
  dataValidResponse?: ImportProductGroupResponseData[];
  dataInvalidResponse?: ImportProductGroupResponseData[];
  validateResponse?: ImportProductGroupResponse;
  ...
  async validateData(): Promise<any> {
    ...
    if (getMessage !== '') {
      ...
      await this.importService
        .validateImport(this.fileData, this.sheetIndex, this.companyId, IMPORT_EXCEL_VALIDATE_PRODUCT_GROUP)
        .subscribe(response => {
          if (response.ok) {
  ...
  viewStatusDetail(item: ImportProductGroupResponseData) {
    if (item) {
  ...
  async completed() {
    ...
    if (this.dataValidResponse && this.dataValidResponse.length > 0) {
      ...
      await this.importService.saveDataImport(request, IMPORT_EXCEL_SAVE_PRODUCT_GROUP).subscribe(
        response => {
          this.isLoading = false;
          if (response.ok) {
            let responseBody = ConvertResponse.getDataFromServer(response, false);
  ...
  downloadErrorData() {
    if (this.dataInvalidResponse) {
      this.exportComponent.exportFileExcel(this.dataInvalidResponse, 'DanhSachNhomSanPhamLoi', this.companyId, EXPORT_EXCEL_PRODUCT_GROUP);
    }
  }
